...
The problem is learning to play the Atari game \textbf{Pong} using reinforcement learning. Pong is a two-player zero sum game where each player controls a vertical paddle and attempts to return a moving ball past the opponent. A point gets scored when the opponent fails to return the ball. The aim of the agent is to maximize the cumulative reward over an episode by learning how to control the paddle optimally.
...
The action space \(\mathcal{A}\) is discrete and consists of the legal paddle movements available to the agent in Pong. These actions include:
...
\begin{enumerate}
    \item High Dimensional State Space: Each state consists of over 28000 pixels (4 $\times$ 84 $\times$ 84).
    \item Observability: A single frame does not have information about motion.
...
\begin{itemize}
    ...
    \item Batch size of 32, 64 and 128
...
We would have welcomed more time to explore other approaches to reinforcement learning and applying them to a broader set of Atari games. 

One of the startling results from the Mnih et al, 2015 paper was the generality of the approach which allowed the same algorithm to be applied to many different games. Pong is one of the simpler games in the Arcade Learning Environment but still requires considerable computational resources to learn which makes evaluating different approaches time consuming. The next step would be to verify the agent when playing more challenging games such as Breakout. Breakout requires many more episodes of training before the agent can acquire superhuman levels of play. 
...
\textbf{[Time spent tuning hyper parameters]}
...
\section*{References}
% 
Mnih, V., Kavukcuoglu, K., Silver, D., Rusu, A.A., Veness, J., Bellemare, M.G., Graves, A., Riedmiller, M., Fidjeland, A.K., Ostrovski, G. and Petersen, S., 2015. \textit{Human-level control through deep reinforcement learning}. Nature, 518(7540), pp.529-533
